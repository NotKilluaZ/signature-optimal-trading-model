...
from src.sigstop.stopping.payoff import build_exit_payoff_process
...
# Build exit-stage features and payoff paths from simulated spreads
def build_exit_training_data(
    ...
    payoffs = build_exit_payoff_process(
        spread_paths,
    ...
    return StoppingTrainingData(
        ...
        metadata = {
            "stage": "exit",
            "payoff_formula": "exp(-r_hat t) * (X - c_hat)",
            "paper_params": {
...
# Load the default exit training dataset from cached OU samples
def load_default_exit_training_data(config: dict[str, Any]) -> StoppingTrainingData:
    ...
    training_data, _, _, _ = load_or_build_synthetic_training_data(
        ...
        builder = lambda: build_exit_training_data(
...
# Train the exit stopping policy using either provided or cached data
    ...
    if spread_paths is not None:
        ...
        if cache_base_dir is not None and cache_source is not None:
            training_data, _, _, _ = load_or_build_synthetic_training_data(
                ...
                source = cache_source,
                builder = lambda: build_exit_training_data(spread_paths, formation_spread, resolved_config),
                extra_metadata = extra_metadata,
            )
        else:
            training_data = build_exit_training_data(spread_paths, formation_spread, resolved_config)
            if extra_metadata is not None:
                training_data.metadata.update(extra_metadata)
    else:
        training_data = load_default_exit_training_data(resolved_config)
